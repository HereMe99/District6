...
/*
 ...
 - 2017/08/08 (1.51) - removed ColorEditMode() and ImGuiColorEditMode in favor of ImGuiColorEditFlags and parameters to the various Color*() functions. The SetColorEditOptions() allows to initialize default but the user can still change them with right-click context menu_back.
...
// Layer is locked for the root window, however child windows may use a different viewport (e.g. extruding menu_back)
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (flags & ImGuiWindowFlags_MenuBar)
        {
            ...
            menu_bar_rect.ClipWith(window->Rect());  // Soft clipping, in particular child window don't have minimum size covering the menu_back bar so this is useful for them.
...
void ImGui::BeginTooltip()
{
    ...
    {
        // The default tooltip position is a little offset to give space to see the context menu_back (it's also clamped within the current viewport/monitor)
...
void ImGui::CloseCurrentPopup()
{
    ...
    // Closing a menu_back closes its top-most parent popup (unless a modal)
    while (popup_idx > 0)
    {
    ...
    // A common pattern is to close a popup when selecting a menu_back item/selectable that will open another window.
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ...
    {
        // Child menus typically request _any_ position within the parent menu_back item, and then we move the NEW menu_back outside the parent bounds.
        // This is how we end up with child menus appearing (most-commonly) on the right of the parent menu_back.
        IM_ASSERT(g.CurrentWindow == window);
        ImGuiWindow* parent_window = g.CurrentWindowStack[g.CurrentWindowStack.Size - 2];
        float horizontal_overlap = g.Style.ItemInnerSpacing.x; // We want some overlap to convey the relative depth of each menu_back (currently the amount of overlap is hard-coded to style.ItemSpacing.x).
        ImRect r_avoid;
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    // 2017/09/29: FIXME: This now currently only enabled inside menu_back bars, ideally we'd disable it everywhere. Menus in particular need to catch failure. For general navigation it feels awkward.
...
static void ImGui::NavUpdate()
{
    ...
    if (nav_keyboard_active)
    {
        ...
        if (g.IO.KeyAlt && !g.IO.KeyCtrl) // AltGR is Alt+Ctrl, also even on keyboards without AltGR we don't want Alt+Ctrl to open menu_back.
    ...
    if (IsNavInputTest(ImGuiNavInput_Cancel, ImGuiInputReadMode_Pressed))
    {
        ...
        {
            // Close open popup/menu_back
            if (!(g.OpenPopupStack.back().Window->Flags & ImGuiWindowFlags_Modal))
                ClosePopupToLevel(g.OpenPopupStack.Size - 1, true);
        }
        else if (g.NavLayer != 0)
        {
            // Leave the "menu_back" layer
...
// Keyboard: CTRL+Tab (change focus/move/resize), Alt (toggle menu_back layer)
// Gamepad:  Hold Menu/Square (change focus/move/resize), Tap Menu/Square (toggle menu_back layer)
static void ImGui::NavUpdateWindowing()
{
    ...
    // Keyboard: Press and Release ALT to toggle menu_back layer
    ...
    if (apply_focus_window && (g.NavWindow == NULL || apply_focus_window != g.NavWindow->RootWindow))
    {
        ...
        // If the window only has a menu_back layer, select it directly
    ...
    // Apply menu_back/layer toggle
    if (apply_toggle_layer && g.NavWindow)
    {
        // Move to parent menu_back if necessary
        ...
        // When entering a regular menu_back bar with the Alt key, we always reinitialize the navigation ID.
