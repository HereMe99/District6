...
void SphereCollider::CollisionPush(Collider* collider)
{
	BoxCollider* coll = dynamic_cast<BoxCollider*>(collider);

	if (coll != nullptr)
		return IsBoxCollisionPush(coll);

	return IsSphereCollisionPush(static_cast<SphereCollider*>(collider));
}
...
void SphereCollider::IsBoxCollisionPush(BoxCollider* collider)
{
	...
	const float distance = Distance(GetWorldPosition(), pos);
	if (distance <= radius)
	{
		Vector3 direction = GetWorldPosition() - pos;
		direction.Normalize();
		direction.y = 0;
		this->target->position += direction * (radius - distance);
	}
}

void SphereCollider::IsSphereCollisionPush(SphereCollider* collider)
{
	const float distance = Distance(GetWorldPosition(), collider->GetWorldPosition());

	if (distance <= radius)
	{
		Vector3 direction = GetWorldPosition() - collider->GetWorldPosition();
		direction.Normalize();
		direction.y = 0;
		this->target->position += direction * (radius - distance);
	}
