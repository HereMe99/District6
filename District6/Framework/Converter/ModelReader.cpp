...
void ModelReader::WriteMeshData(string savePath, bool isOverwrite)
{
	...
	Xml::XMLElement* root = document->NewElement("model");
	document->LinkEndChild(root);
	Xml::XMLElement* node = document->NewElement("bones");
	root->LinkEndChild(node);
	node->SetAttribute("size", bones.size());
	...
	for (AssimpBone* bone : bones)
	{
		...
		Xml::XMLElement* element = document->NewElement("Name");
		element->SetText(bone->name.c_str());
		node->LinkEndChild(element);
	...
	node = document->NewElement("meshes");
	root->LinkEndChild(node);
	node->SetAttribute("size", meshes.size());
	...
	for (AssimpMesh* mesh : meshes)
	{
		...
		Xml::XMLElement* node = document->NewElement("vertices"+ind);
		root->LinkEndChild(node);
		node->SetAttribute("size", mesh->vertices.size());
		
		w->UInt(mesh->vertices.size());
		w->Byte(mesh->vertices.data(), sizeof(ModelVertex) * mesh->vertices.size());

		node = document->NewElement("indices"+ ind);
		root->LinkEndChild(node);
		node->SetAttribute("size", mesh->indices.size());
	...
	string matPath = Path::GetDirectoryName(savePath) + Path::GetFileNameWithoutExtension(savePath);
	matPath += ".xml";
	document->SaveFile(matPath.c_str());
	delete document;
	delete w;
}
