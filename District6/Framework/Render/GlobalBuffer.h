#pragma once
#include "Framework.h"

class ViewProjectionBuffer : public ConstBuffer
{
private:
	struct Data
	{
		Matrix view;
		Matrix projection;
		Matrix invView;
	}data;

public:
	ViewProjectionBuffer() : ConstBuffer(&data, sizeof(Data))
	{
		data.view = XMMatrixIdentity();
		data.projection = XMMatrixIdentity();
		data.invView = XMMatrixIdentity();
	}

	void SetView(Matrix value)
	{
		data.view = XMMatrixTranspose(value);
		data.invView = XMMatrixTranspose(XMMatrixInverse(nullptr, value));
	}

	void SetProjection(Matrix value)
	{
		data.projection = XMMatrixTranspose(value);
	}
};

class WorldBuffer : public ConstBuffer
{
private:
	struct Data
	{
		Matrix world;
	}data;

public:
	WorldBuffer() : ConstBuffer(&data, sizeof(Data))
	{
		data.world = XMMatrixIdentity();		
	}

	void SetWorld(Matrix value)
	{
		data.world = XMMatrixTranspose(value);
	}
};

class ColorBuffer : public ConstBuffer
{
public:
	struct Data
	{
		Float4 color;
	}data;

	ColorBuffer() : ConstBuffer(&data, sizeof(Data))
	{
		data.color = Float4(1, 1, 1, 1);
	}
};

class LightBuffer : public ConstBuffer
{
public:
	struct Data
	{
		Float3 direction;
		float specExp;

		Float4 ambient;		
		Float4 ambientFloor;

		float lightPower;
		Float3 padding;
	}data;

	LightBuffer() : ConstBuffer(&data, sizeof(Data))
	{
		data.direction = Float3(0, -1, -0.5f);
		data.specExp = 64.0f;
		data.ambient = Float4(0.05f, 0.05f, 0.05f, 1.0f);
		data.ambientFloor = Float4(0.05f, 0.05f, 0.05f, 1.0f);
		data.lightPower = 0.15f;
	}
};

class IntBuffer : public ConstBuffer
{
public:
	struct Data
	{
		int indices[4];
	}data;

	IntBuffer() : ConstBuffer(&data, sizeof(Data))
	{
		for (int& i : data.indices)
			i = 0;
	}
};

class ShadowBuffer : public ConstBuffer
{
public:
	struct Data
	{
		Float2 mapSize;
		float bias;
		int quality;
	}data;

	ShadowBuffer() : ConstBuffer(&data, sizeof(Data))
	{
		data.mapSize = Float2(0, 0);
		data.bias = -0.0006f;
		data.quality = 0;
	}
};

struct LightInfo
{
	enum Type
	{
		DIRECTION,
		POINT,
		SPOT,
		CAPSULE
	};

	Float4 color;

	Float3 position;
	float range;

	Float3 direction;
	float outer;

	float inner;
	float length;
	Type type;

	float power;

	LightInfo()
	{
		type = POINT;

		color = Float4(0.1f, 0.1f, 0.1f, 1.0f);
		position = Float3(0, 0, 0);
		range = 80.0f;

		direction = Float3(0, -1, 0);
		outer = 65;
		inner = 55;

		length = 50;
		power = 1.0f;
	}
};

#define MAX_LIGHT 10

class LightInfoBuffer : public ConstBuffer
{
public:
	struct Data
	{
		LightInfo lights[MAX_LIGHT];

		UINT lightCount;
		float padding[3];
	}data;

	LightInfoBuffer() : ConstBuffer(&data, sizeof(Data))
	{
		data.lightCount = 0;
	}

	int Add(LightInfo light)
	{
		const int result = data.lightCount;
		data.lights[data.lightCount] = light;
		data.lightCount++;
		return result;
	}
};