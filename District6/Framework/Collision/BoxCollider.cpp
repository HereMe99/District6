	...
	: minBox(min), maxBox(max)
{
	...
	vertexBuffer = NEW VertexBuffer(vertices.data(), sizeof(Vertex), vertices.size(),true);
...
void BoxCollider::CollisionPush(Collider* collider)
{
	BoxCollider* coll = dynamic_cast<BoxCollider*>(collider);
...
void BoxCollider::SetBox(const Float3 min, const Float3 max)
{
	minBox = min;
	maxBox = max;
...
void BoxCollider::IsBoxCollisionPush(BoxCollider* collider)
{
	/*Obb box1 = GetObb();
	Obb box2 = collider->GetObb();
...
void BoxCollider::IsSphereCollisionPush(SphereCollider* collider)
{
	return collider->IsBoxCollisionPush(this);
}
...
bool BoxCollider::IsPointCollision(Vector3 position)
{
	SphereCollider* sphere = NEW SphereCollider(1);
	bool result = IsSphereCollision(sphere);
	SAFE_DELETE(sphere);
	return result;
}
