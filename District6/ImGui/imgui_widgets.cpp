...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    {
        // Vertical separator, for menu_back bars (use current line height). Not exposed because it is misleading and it doesn't have an effect on regular layout.
...
// With typical options: Left-click on colored square to open color picker. Right-click to open option menu_back. CTRL-Click over input fields to edit them and TAB to go to next item.
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // Context menu_back: display and modify options (before defaults are applied)
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Context menu_back: display and store options.
    if (!(flags & ImGuiColorEditFlags_NoOptions))
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // We use NoHoldingActiveID on menus so user can click and _hold_ on a menu_back then drag to browse child entries
    ImGuiButtonFlags button_flags = 0;
...
// Currently the main responsibility of this function being to setup clip-rect + horizontal layout + menu_back navigation layer.
// Ideally we also want this to be responsible for claiming space out of the main window scrolling rectangle, in which case ImGuiWindowFlags_MenuBar will become unnecessary.
// Then later the same system could be used for multiple menu_back-bars, scrollbars, side-bars.
...
void ImGui::EndMenuBar()
{
    ...
    // Nav: When a move request within one of our child menu_back failed, capture the request to navigate among our siblings.
...
// For the main menu_back bar, which cannot be moved, we honor g.Style.DisplaySafeAreaPadding to ensure text can be visible on a TV set.
...
void ImGui::EndMainMenuBar()
{
    ...
    // When the user has left the menu_back layer (typically: closed menus through activation of an item), we restore focus to the previous window
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (menuset_is_open)
        g.NavWindow = window;  // Odd hack to allow hovering across menus of a same menu_back-set (otherwise we wouldn't be able to hover parent)

    // The reference position stored in popup_pos will be used by Begin() to find a suitable position for the child menu_back,
    // However the final position is going to be different! It is choosen by FindBestWindowPosForPopup().
    // e.g. Menus tend to overlap each other horizontally to amplify relative Z-ordering.
    ImVec2 popup_pos, pos = window->DC.CursorPos;
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        // Menu inside an horizontal menu_back bar
    ...
    {
        // Close menu_back when not hovering it anymore unless we are moving roughly in the direction of the menu_back
    ...
    {
        // Menu bar
        if (menu_is_open && pressed && menuset_is_open) // Click an open menu_back again to close it
        {
            want_close = true;
    ...
    if (!enabled) // explicitly close if an open menu_back becomes disabled, facilitate users code a lot in pattern such as 'if (BeginMenu("options", has_object)) { ..use object.. }'
        want_close = true;
    ...
    {
        // Don't recycle same menu_back level in the same frame, first close the other menu_back and yield for a frame.
        OpenPopup(label);
        return false;
    }
    ...
    if (menu_is_open)
    {
        // Sub-menus are ChildWindow so that mouse can be hovering across them (otherwise top-most popup menu_back would steal focus and not allow hovering on parent menu_back)
...
void ImGui::EndMenu()
{
    // Nav: When a left move request _within our child menu_ failed, close ourselves (the _parent_ menu_back).
    // A menu_back doesn't close itself because EndMenuBar() wants the catch the last Left<>Right inputs.
    // However, it means that with the current code, a BeginMenu() from outside another menu_back or a menu_back-bar won't be closable with the Left direction.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        // Mimic the exact layout spacing of BeginMenu() to allow MenuItem() inside a menu_back bar, which is a little misleading but may be useful
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // If the user called us with *p_open == false, we early out and don't render. We make a dummy call to ItemAdd() so that attempts to use a contextual popup menu_back with an implicit ID won't use an older ID.
    if (p_open && !*p_open)
    {
    ...
    // Select with right mouse button. This is so the common idiom for context menu_back automatically highlight the current widget.
