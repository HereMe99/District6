...
namespace ImGui
{
    ...
    IMGUI_API void          SetNextWindowContentSize(const ImVec2& size);                               // set next window content size (~ scrollable client area, which enforce the range of scrollbars). Not including window decorations (title bar, menu_back bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
    ...
    IMGUI_API void          Separator();                                                    // separator, generally horizontal. inside a menu_back bar or in horizontal layout mode, this becomes a vertical separator.
    ...
    // Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little colored preview square that can be left-clicked to open a picker, and right-clicked to open an option menu_back.)
    ...
    // Widgets: Menus
    // - Use BeginMenuBar() on a window ImGuiWindowFlags_MenuBar to append to its menu_back bar.
    // - Use BeginMainMenuBar() to create a menu_back bar at the top of the screen.
    IMGUI_API bool          BeginMenuBar();                                                     // append to menu_back-bar of current window (requires ImGuiWindowFlags_MenuBar flag set on parent window).
    IMGUI_API void          EndMenuBar();                                                       // only call EndMenuBar() if BeginMenuBar() returns true!
    IMGUI_API bool          BeginMainMenuBar();                                                 // create and append to a full screen menu_back-bar.
    IMGUI_API void          EndMainMenuBar();                                                   // only call EndMainMenuBar() if BeginMainMenuBar() returns true!
    IMGUI_API bool          BeginMenu(const char* label, bool enabled = true);                  // create a sub-menu_back entry. only call EndMenu() if this returns true!
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_MenuBar                = 1 << 10,  // Has a menu_back-bar
...
enum ImGuiNavInput_
{
    ...
    ImGuiNavInput_Menu,          // tap: toggle menu_back / hold: focus, move, resize // e.g. Square (PS4), X (Xbox), Y (Switch), Alt (Keyboard)
    ...
    ImGuiNavInput_KeyMenu_,      // toggle menu_back                                  // = io.KeyAlt
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoOptions       = 1 << 3,   //              // ColorEdit: disable toggling options menu_back when right-clicking on inputs/small preview.
    ...
    // override them in most of your calls. Let the user choose via the option menu_back and/or call SetColorEditOptions() once during startup.
