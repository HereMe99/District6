...
enum ImGuiSeparatorFlags_
{
    ...
    ImGuiSeparatorFlags_Horizontal          = 1 << 0,   // Axis default to current layout type, so generally Horizontal unless e.g. in a menu_back bar
...
struct ImGuiPopupData
{
    ...
    ImGuiID             OpenParentId;   // Set on OpenPopup(), we need this to differentiate multiple menu_back sets from each others (e.g. inside menu_back bar vs loose menu_back items)
...
struct ImGuiContext
{
    ...
    ImGuiNavLayer           NavLayer;                           // Layer we are navigating on. For now the system is hard-coded for 0=main contents and 1=menu_back/title bar, may expose layers later.
    ...
    ImGuiNavForward         NavMoveRequestForward;              // None / ForwardQueued / ForwardActive (this is used to navigate sibling parent menus from a child menu_back)
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2                  MenuBarOffset;          // MenuBarOffset.x is sort of equivalent of a per-layer CursorPos.x, saved/restored as we switch to the menu_back bar. The only situation when MenuBarOffset.y is > 0 if when (SafeAreaPadding.y > FramePadding.y), often used on TVs.
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiID                 PopupId;                            // ID in the popup stack when this window is used as a popup/menu_back (because we use generic Name/ID for recycling)
    ...
    ImRect                  InnerRect;                          // Inner rectangle (omit title bar, menu_back bar, scroll bar)
    ...
    ImGuiMenuColumns        MenuColumns;                        // Simplified columns storage for menu_back items
    ...
    ImGuiWindow*            NavLastChildNavWindow;              // When going to the menu_back bar, we remember the child window we came from. (This could probably be made implicit if we kept g.Windows sorted by last focused including child window.)
